...
class AAEncoder(MessagePassing):
    ...
    def __init__(self,
        ...
        self.center_embed = nn.SingleInputEmbedding(in_channel = node_dim, out_channel = embed_dim)
        self.nbr_embed = nn.MultipleInputEmbedding(in_channel = [node_dim, edge_dim], out_channel = embed_dim)
    ...
    def forward(self,
        ...
        center_embed = center_embed + self._mha_block(x = x, 
        ...
        center_embed = center_embed + self._ff_block(self.layer_norm2(center_embed))
    ...
    def message(self,
        ...
        if rotate_mat is None:
            ...
            nbr_embed = self.nbr_embed([torch.bmm(x_j.unsqueeze(-2), center_rotate_mat),
        ...
        value = self.lin_q(nbr_embed).view(-1, self.num_heads, self.embed_dim//self.num_heads)
    ...
    def _mha_block(self,
        ...
        center_embed = self.out_proj(self.propogate(center_embed = center_embed, 
                                      x = x, 
                                      edge_index = edge_index,
                                      edge_attr = edge_attr,
                                      rotate_mat = rotate_mat,
...
class TemporalEncoder(nn.Module):
    def __init__(self,
        ...
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer = encoder_layer, num_layers = encoder_layer,
                                                         norm = nn.LayerNorm(embed_dim))
        self.historical_steps = historical_steps
        self.padding_token = nn.Parameter(torch.Tensor(self.historical_steps, 1, embed_dim))
        self.cls_token = nn.Parameter(torch.Tensor(1,1,embed_dim))
        self.pos_embed = nn.Parameter(torch.Tensor(self.historical_steps+1,1,embed_dim))
...
class TemporalEncoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        x = src
        x = x + self._sa_block(self.layer_norm1(x), src_mask, src_key_padding_mask)
