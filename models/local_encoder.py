...
class AAEncoder(MessagePassing):
    ...
    def message(
        ...
        value = self.lin_q(nbr_embed).view(
            -1, self.num_heads, self.embed_dim // self.num_heads
        )
...
class TemporalEncoder(nn.Module):
    def __init__(
        ...
        self.padding_token = nn.Parameter(
            torch.Tensor(self.historical_steps, 1, embed_dim)
        )
        self.cls_token = nn.Parameter(torch.Tensor(1, 1, embed_dim))
        self.pos_embed = nn.Parameter(
            torch.Tensor(self.historical_steps + 1, 1, embed_dim)
        )
...
class ALEncoder(MessagePassing):
    ...
    def forward(
        ...
        x_actor = x_actor + self._ff_block(self.layer_norm2(x_actor))
        return x_actor
