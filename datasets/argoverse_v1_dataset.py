...
class ArgoverseV1Dataset(Dataset):
    ...
    def __init__(
        ...
        self._url = (
            f"https://s3.amazonaws.com/argoai-argoverse/forecasting_{split}_v1.1.tar.gz"
        )
        if split == "sample":
...
def process_argoverse(
    split: str, raw_path: str, am: ArgoverseMap, radius: float
) -> Dict:
    ...
    av_heading_vector = origin - torch.tensor(
        [av_df[18]["X"], av_df[18]["Y"]], dtype=torch.float
    )
    ...
    for actor_id, actor_df in df.groupby("TRACK_ID"):
        ...
        if (
            ...
            # NOTE: get current heading of the actor
...
def get_lane_features(
    ...
    lane_actor_index = (
        torch.LongTensor(list(product(torch.arange(lane_vectors.size(0)), node_inds)))
        .t()
        .contiguous()
    )
